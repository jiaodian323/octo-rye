...
package com.justnd.octoryeclient.music.utils;
...
//     * musicID. This is necessary so we know where the user selected the music from, when the
//     music
...
//     * mediaID. This is necessary so we know where the user selected the music from, when the
//     music
//     * exists in more than one music list, and thus we are able to correctly build the playing
//     * queue.
//     * 通过mediaID获取类型和类型信息
...
//     */
//    public static boolean isMediaItemPlaying(Activity context,
...
//    }
    ...
    public static String createMusicId(String audioURL) {
        return String.valueOf(audioURL.hashCode());
    }

    public static String getMusicIdPlaying(Activity context) {
        MediaControllerCompat controller = MediaControllerCompat.getMediaController(context);
        if (controller != null && controller.getMetadata() != null) {
            return controller.getMetadata().getDescription().getMediaId();
        }
        return null;
    }
}
